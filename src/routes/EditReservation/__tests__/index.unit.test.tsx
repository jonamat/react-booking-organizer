 ...
 * Unit test suite
 *
 * Target: EditReservation component
 * Tests:
 * - Responses handling
 * - Redirection
 ...
 * - Functionality
 *
 * For data validation, API call and errors handling see {@link '../../../utils/editReservation'}
 */
...
jest.mock('../../../utils', () => ({
    ...
    editReservation: jest.fn(() => Promise.resolve({ id: 'fakeID', date: new Date('1 Jan 2040 10:00 GMT+0100') })),
}));
...
const renderComponent = (props?) => {
    return render(
        <MuiPickersUtilsProvider utils={DateFnsUtils}>
            <EditReservation history={history} location={{}} match={match} {...props} />
...
describe('EditReservation component', () => {
    describe('Upload system', () => {
        it('calls the db middleware function with correct user inputs', () => {
            ...
            expect(editReservation as jest.Mock).toBeCalledTimes(1);

            expect((editReservation as jest.Mock).mock.calls[0][0]).toEqual(__mockReservations[0].id);
            expect(Array.from((editReservation as jest.Mock).mock.calls[0][1].entries())).toEqual([
                ['serviceId', __mockServices[0].id],
                ['employeeId', __mockEmployees[0].id],
                ['notes', 'notes'],
            ]);
            expect((editReservation as jest.Mock).mock.calls[0][2]).toEqual(__mockCustomers[0]);
            expect((editReservation as jest.Mock).mock.calls[0][3]).toEqual(parsedDate);
            expect((editReservation as jest.Mock).mock.calls[0][4]).toEqual({
                employees: __mockEmployees,
                services: __mockServices,
                reservations: __mockReservations,
            });
            expect((editReservation as jest.Mock).mock.calls[0][5]).toBeUndefined();
        });
        ...
        it('parses and loads queries in url', () => {
            ...
            expect((getByTestId('date') as HTMLInputElement).value).toBe('01/01/2000 10:00');
        });
    });
    ...
    describe('User experience', () => {
        ...
        it('calls toast on error', async () => {
            (editReservation as jest.Mock).mockImplementation(() => Promise.reject('some error'));
